<div class="header--menu burger-button">
    <div class="btn-layers"></div>
</div>
<div class="l-container burger-menu">
    <nav role="navigation" aria-labelledby="block-burgermenu-menu" id="block-burgermenu" class="block block-menu navigation menu--burger">
        <ul class="menu">
            <li class="menu-item menu-item--expanded">
                <a href="/" data-drupal-link-system-path="<front>" >Event</a>
                <ul class="menu sublinks">
                    <li class="menu-item">
                        <a href="/node/4" data-drupal-link-system-path="node/4">About Us</a>
                    </li>
                    <li class="menu-item">
                        <a href="/node/4" data-drupal-link-system-path="node/4">Important dates</a>
                    </li>
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">About the Drupal Association Romania</a>
                    </li>
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">News</a>
                    </li>
                </ul>
            </li>
            <li class="menu-item menu-item--active-trail">
                <a href="/user/register" data-drupal-link-system-path="user/register" class="is-active">Registration</a>
            </li>
            <li class="menu-item menu-item--expanded">
                <a href="#" data-drupal-link-system-path="<front>" >Program</a>
                <ul class="menu sublinks">
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">Schedule</a>
                    </li>
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">Birds of a Feather</a>
                    </li>
                    <li class="menu-item">
                        <a href="/node/4" data-drupal-link-system-path="node/4">Sprints</a>
                    </li>
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">Social Events</a>
                    </li>
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">Accepted Sessions</a>
                    </li>
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">Session Tracks</a>
                    </li>
                </ul>
            </li>
            <li class="menu-item menu-item--expanded">
                <a href="/" data-drupal-link-system-path="<front>" >Travel</a>
                <ul class="menu sublinks">
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">Venue</a>
                    </li>
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">Visit Cluj-Napoca</a>
                    </li>
                </ul>
            </li>
            <li class="menu-item menu-item--collapsed">
                <a href="/" data-drupal-link-system-path="<front>" >Community</a>
            </li>
            <li class="menu-item menu-item--expanded">
                <a href="/" data-drupal-link-system-path="<front>" >Sponsors</a>
                <ul class="menu sublinks">
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">Become a sponsor</a>
                    </li>
                    <li class="menu-item">
                        <a href="/" data-drupal-link-system-path="<front>">Our sponsors</a>
                    </li>
                </ul>
            </li>
            <li class="menu-item">
                <a href="/" data-drupal-link-system-path="<front>" >Login</a>
            </li>
        </ul>
    </nav>
</div>
